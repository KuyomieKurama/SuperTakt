...
### Welle vom 2026-09-04, Welle G (Buchung von Hand, Reviewer-Hinweise, End-to-End nachziehen) — fertig, Gesamtlauf und Review offen
...
| O-X | `PATCH /time-entries/{id}` erlaubt `todoId`; das Umhängen legt die Achse „hat offene Buchungen" an **zwei** Todos in entgegengesetzte Richtungen um. Bewußt ohne `poolMovement` (E-061 Nachtrag); soll die Oberfläche dort etwas sagen, braucht es eine Form für zwei Bewegungen | Auftraggeber |
| O-Y | Soll der Toast nach der Buchung von Hand wie der nach dem Stopp auch Tagesgruppe und gerundeten Exportwert nennen (T-108 Frage 1)? | Auftraggeber |
