...
| `ix_todo_attachment_email` (partiell, `(target) WHERE origin = 'email'`, seit 0023) | Dieselbe Frage für die aus E-Mails übernommenen Dateien (A-19.23, A-A-83). `origin = 'email'` allein und nicht zusätzlich `kind = 'file'`: Ein Cloud-Verweis (A-19.25) hat dieselbe Herkunft, kostet im Index eine Zeile und schadet nicht; eine Bedingung über zwei Spalten machte den Index enger und die Abfrage nicht schneller |
