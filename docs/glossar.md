...
| Call-Nummer | „Call", „callNumber" | Spalte `todo.call_number`; Exportschlüssel `Call`; Quellenpfad `todo.callNumber` | Die Nummer, über die ein Vorgang beim Abrechnungstool identifiziert wird. Das Outlook-Add-in erkennt sie per konfigurierbarem regulärem Ausdruck. Darf leer bleiben; nicht eindeutig erzwungen, weil der Benutzer selbst entscheidet, ob er ein bereits gefundenes Todo in SuperTakt weiterbearbeitet oder aus dem Add-in heraus bewusst ein zweites anlegt — das Add-in bietet am gefundenen Todo keine Handlung an (A-10.9, E-100). | A-2.6, A-8.2, A-8.7, A-10.8, A-10.9, E-100, docs/datenmodell.md §3.2 |
...
| Outlook-Add-in, kurz Add-in | kein weiterer Name in der Spezifikation | `apps/outlook-addin` | Der Teil von Takt, der in Outlook läuft und aus einer E-Mail heraus ausschließlich ein neues Todo anlegt. Existiert bereits ein Todo mit derselben Call-Nummer, weist das Add-in nur darauf hin — es bietet dort weder eine Zeitbuchung noch einen Anhang an (A-10.9, E-100). Spricht ausschließlich mit dem lokalen Dienst, über eine eigens dafür geschnittene, schmale Routengruppe (`/api/v1/addin/*`), die keine Anhangroute führt. | A-10.1, A-10.9, A-19.19, E-100 |
