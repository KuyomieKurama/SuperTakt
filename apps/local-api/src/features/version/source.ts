 ...
 * Fehlschlag wie jeder andere: kein Hinweis, keine Fehlerfläche, kein
 * wiederholtes Nachfragen **im selben Prüflauf** (A-18.11). Der Takt selbst
 * läuft weiter — seit T-273 plant ein Fehlschlag auf den Mindestabstand von
 * einer Stunde neu und beendet die Prüfung **nicht** für die Laufzeit
 * (höchstens 24 Anfragen je Kalendertag im Dauerfehlschlag gegen 1 je 24
 * Stunden im Erfolgsfall).
 *
 * Ohne Ergebnis bleibt die Prüfung trotzdem, und zwar dauerhaft: Die Ursache
 * ist die Antwort und nicht der Zeitgeber, also liest jeder weitere Prüflauf
 * dieselbe zu große Antwort und schreibt dieselbe Protokollzeile, die niemand
 * liest. Das ist der eigentliche Befund: nicht daß die Grenze knapp ist,
 * sondern daß ihr Überschreiten aussieht wie „alles aktuell".
 *
