...
describe("Meldeflächen stehen unbedingt im Baum (O-GQ, T-191)", () => {
  it("ExportSettings.tsx — die Meldefläche des Exportordners", () => {
    const { sourceFile } = parse("../src/features/settings/ExportSettings.tsx");
  ...
  it("ExportDirectoryField.tsx — der Behälter trägt die Rolle, nicht der bedingte Absatz darin", () => {
    const { sourceFile } = parse("../src/features/export/ExportDirectoryField.tsx");
  ...
  it("TemplateFieldRow.tsx — derselbe Griff, ohne eigene Klasse am Behälter", () => {
    const { sourceFile } = parse("../src/features/export/TemplateFieldRow.tsx");
