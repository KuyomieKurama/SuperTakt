 ...
 * ---------------------------------------------------------------------------
 *
 * Die Oberfläche ruft die Domäne. Was hier bleibt, ist der **Übergang** — die
 * Zeichenketten dieser Oberfläche hinein, `ForeignText` heraus —, genau die
 * Bauart, die `lib/deadline.ts` für `dueState` schon vormacht. Wer die
