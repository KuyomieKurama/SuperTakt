/**
 * Takt — Pools und Kanban-Spalten: die Regel und ihre Auswertung
 * (A-3.*, A-2.4, A-2.5, E-054 bis E-057).
 *
 * ---------------------------------------------------------------------------
 * Warum das eine eigene Datei ist (T-261)
 * ---------------------------------------------------------------------------
 *
 * Bis T-261 stand alles hier zusammen mit den Tags und den Tag-Ordnern in
 * `tag.ts`. Der Name war schon damals zu eng: Eine Regel nennt seit E-055 fünf
 * Achsen, und **vier davon sind keine Tags** — ausgeschlossene Tags, Status,
 * Erledigt, Exportstatus. Wer den Exportstatusfilter suchte, suchte ihn in
 * einer Datei, die „Tags" heißt.
 *
 * Der Schnitt ist nicht erfunden, er stand ausgeschrieben: eigene
 * Anforderungen (A-3 gegen A-4 und A-9), eigene Tabellen (`pool`/`pool_rule`
 * gegen `tag_folder`, `tag`, `default_tag`) — und **keine einzige
 * Verwendung** in die eine oder andere Richtung. Diese Datei nimmt aus
 * `tag.ts` nichts mit; sie kennt von dort weder `Tag` noch `TagFolder`,
 * sondern allein die Kennungstypen aus `kernel.ts`. Genau das
 * ist der Punkt aus {@link PoolTagTerm}: Ein Ordnerterm ist eine bequeme
 * Schreibweise für „diese Tags", und wer die Regel auswertet, sieht am Ende
 * eine Tagmenge und keinen Ordner mehr.
 *
 * ---------------------------------------------------------------------------
 * Was hier **nicht** steht
 * ---------------------------------------------------------------------------
 *
 *  - **Das Auflösen der Ordner.** Dafür braucht es den Baum, also einen Port
 *    (`PoolPort.resolveAxes`). Alles hier ist rein: Die aufgelösten Taglisten
 *    kommen als Eingabe herein.
 *  - **Die Mitgliederabfrage.** Sie steht als SQL in `packages/storage` und
 *    urteilt über dasselbe Gebilde ({@link ResolvedPoolRuleAxes}), damit die
 *    beiden Urteile nicht auseinanderlaufen können.
 *  - **Der Satz über eine Bewegung.** `pool-movement.ts`.
 *  - **Die Board-Ansicht.** `board.ts` — sie erbt ihre Regelseite von
 *    {@link MatchesPoolRule}, statt sie abzuschreiben (T-089).
 *
