...
it('A-21.4: speichert Gestaltung und Dichte unabhängig vom Farbmodus über einen Neustart', async () => {
  ...
  try {
    ...
    await database.transactions.inTransaction(async (unit) => {
      expect(await unit.settings.load()).toMatchObject({ designTheme: 'classic', density: 'comfortable' });
      expect((await unit.settings.update({ theme: 'dark', designTheme: 'clear', density: 'compact', now: NOW })).ok).toBe(true);
    ...
    await database.migrations.migrateToLatest();
    await database.transactions.inTransaction(async (unit) => {
      expect(await unit.settings.load()).toMatchObject({ theme: 'light', designTheme: 'clear', density: 'compact' });
    });
  } finally {
    database.close();
...
it('A-21.4: bestehende Daten erhalten Klassisch, ihren Farbmodus und ihre Todos', async () => {
  ...
  try {
    ...
    expect(await database.unit.settings.load()).toMatchObject({ theme: 'dark', designTheme: 'classic', density: 'comfortable' });
