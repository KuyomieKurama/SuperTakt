...
it('A-21.4: speichert Gestaltung und Dichte unabhängig vom Farbmodus über einen Neustart', async () => {
  ...
  try {
    ...
    await database.transactions.inTransaction(async (unit) => {
      expect(await unit.settings.load()).toMatchObject({ designTheme: 'clear', density: 'comfortable', promptOnTimerStop: true });
      expect((await unit.settings.update({ theme: 'dark', designTheme: 'classic', density: 'compact', promptOnTimerStop: false, now: NOW })).ok).toBe(true);
    ...
    await database.migrations.migrateToLatest();
    await database.transactions.inTransaction(async (unit) => {
      expect(await unit.settings.load()).toMatchObject({ theme: 'light', designTheme: 'classic', density: 'compact', promptOnTimerStop: false });
    });
  } finally {
    database.close();
